...
class Component(CustomComponent):
    ...
    def _serialize_for_executor(self) -> dict[str, Any]:
        ...
        Returns:
            Dictionary containing component state for remote execution
        """
        # Get the actual module path where the class is defined FIRST
        component_class = self.__class__
        component_class_name = component_class.__name__
        component_module = component_class.__module__
        
        logger.info(
            f"[SERIALIZE] Component {component_class_name} has module: {component_module}"
        )
        
        # If module is lfx.custom.validate or similar, try to find the real module
        if component_module in ("lfx.custom.validate", "lfx.custom.custom_component.component"):
            logger.info(
                f"[SERIALIZE] Module path is incorrect ({component_module}), attempting to find correct module"
            )
            # Use inspect to get the actual file where the class is defined
            try:
                import importlib
                import os
                
                # Get the file where the class is actually defined
                class_file = None
                try:
                    class_file = inspect.getfile(component_class)
                    logger.info(f"[SERIALIZE] Class file path: {class_file}")
                except (OSError, TypeError) as e:
                    logger.warning(f"[SERIALIZE] Could not get file path using inspect.getfile: {e}")
                    # Try alternative: check if class has __file__ attribute
                    if hasattr(component_class, "__file__"):
                        class_file = component_class.__file__
                        logger.info(f"[SERIALIZE] Got file path from __file__: {class_file}")
                    else:
                        # Try to get from the module
                        try:
                            mod = inspect.getmodule(component_class)
                            if mod and hasattr(mod, "__file__"):
                                class_file = mod.__file__
                                logger.info(f"[SERIALIZE] Got file path from module: {class_file}")
                        except Exception:
                            pass
                
                # First, try to derive module from file path (most reliable)
                if class_file:
                    # File path example: /app/src/lfx/src/lfx/components/input_output/text.py
                    # Module should be: lfx.components.input_output.text
                    if "lfx" in class_file:
                        # Find the lfx/src or lfx directory in the path
                        parts = class_file.split(os.sep)
                        try:
                            # Look for 'lfx' directory in path
                            lfx_idx = next(i for i, p in enumerate(parts) if p == "lfx")
                            # Get path after lfx/src/ or lfx/
                            if lfx_idx + 1 < len(parts) and parts[lfx_idx + 1] == "src":
                                module_parts = parts[lfx_idx + 2:-1]  # Skip 'lfx', 'src', and filename
                            else:
                                module_parts = parts[lfx_idx + 1:-1]  # Skip 'lfx' and filename
                            
                            # Remove 'src' if it's in the path
                            if "src" in module_parts:
                                module_parts = [p for p in module_parts if p != "src"]
                            
                            # Build module path
                            if module_parts:
                                potential_module = "lfx." + ".".join(module_parts)
                                # Verify the module exists and has the class
                                try:
                                    mod = importlib.import_module(potential_module)
                                    if hasattr(mod, component_class_name):
                                        component_module = potential_module
                                        logger.info(
                                            f"[SERIALIZE] Found module {component_module} for {component_class_name} from file path {class_file}"
                                        )
                                except Exception as e:
                                    logger.debug(f"[SERIALIZE] Could not import {potential_module}: {e}")
                                    pass
                        except (StopIteration, IndexError):
                            pass
                else:
                    logger.debug("[SERIALIZE] No class file available, skipping file path detection")
                
                # If still not found, try to find the module by checking the MRO
                # But only check modules that actually contain the class name
                if component_module in ("lfx.custom.validate", "lfx.custom.custom_component.component"):
                    logger.info(f"[SERIALIZE] Trying MRO search for {component_class_name}")
                    for cls in component_class.__mro__:
                        if cls.__module__ and not cls.__module__.startswith("lfx.custom"):
                            logger.debug(f"[SERIALIZE] Checking MRO class {cls.__name__} in module {cls.__module__}")
                            # Check if this class is actually defined in that module
                            try:
                                mod = importlib.import_module(cls.__module__)
                                if hasattr(mod, component_class_name):
                                    component_module = cls.__module__
                                    logger.info(
                                        f"[SERIALIZE] Found module {component_module} for {component_class_name} from MRO (class {cls.__name__})"
                                    )
                                    break
                            except Exception as e:
                                logger.debug(f"[SERIALIZE] Could not check MRO module {cls.__module__}: {e}")
                                continue
                    
                    # If still not found, log a warning
                    if component_module in ("lfx.custom.validate", "lfx.custom.custom_component.component"):
                        logger.error(
                            f"[SERIALIZE] FAILED to find correct module for {component_class_name}. "
                            f"Module path detection failed. MRO classes: {[c.__name__ for c in component_class.__mro__]}"
                        )
            except Exception as e:
                logger.debug(f"Could not determine module path from file: {e}")

        # Now get component code
        component_code = None
        
        # ALWAYS try to get code directly from the class first (most reliable)
        try:
            component_code = inspect.getsource(component_class)
            logger.info(f"[SERIALIZE] Got source code directly for {component_class_name} from class")
        except (OSError, TypeError) as e:
            logger.debug(f"[SERIALIZE] Could not get source directly: {e}")
            
            # If direct source fails, try to get from the correct module (if we have it)
            if component_module and component_module not in ("lfx.custom.validate", "lfx.custom.custom_component.component"):
                try:
                    import importlib
                    mod = importlib.import_module(component_module)
                    if hasattr(mod, component_class_name):
                        # Get the entire module source (includes imports)
                        component_code = inspect.getsource(mod)
                        logger.info(f"[SERIALIZE] Got module source code for {component_class_name} from {component_module}")
                    else:
                        logger.warning(f"[SERIALIZE] Module {component_module} does not have class {component_class_name}")
                except Exception as e2:
                    logger.warning(f"[SERIALIZE] Could not get code from module {component_module}: {e2}")
            
            # If module path is wrong, try to find the actual component file
            if not component_code:
                try:
                    # Try to get the actual component file by importing the correct module
                    # Component name maps to module path: TextInput -> lfx.components.input_output.text
                    component_name = getattr(self, "name", None) or getattr(component_class, "name", None)
                    
                    # Build potential module paths based on class name and component name
                    potential_modules = []
                    
                    # Based on class name: TextInputComponent -> TextInput -> text
                    if component_class_name.endswith("Component"):
                        base_name = component_class_name[:-9]  # Remove "Component"
                        # TextInput -> text, ChatInput -> chat, etc.
                        if base_name.endswith("Input"):
                            file_name = base_name[:-5].lower()  # Remove "Input", lowercase
                            potential_modules.append(f"lfx.components.input_output.{file_name}")
                        potential_modules.append(f"lfx.components.input_output.{base_name.lower()}")
                    
                    # Based on component name attribute
                    if component_name:
                        # TextInput -> try text.py (common pattern)
                        if component_name == "TextInput":
                            potential_modules.insert(0, "lfx.components.input_output.text")
                        potential_modules.append(f"lfx.components.input_output.{component_name.lower()}")
                    
                    # Always try the known location for TextInputComponent
                    if component_class_name == "TextInputComponent":
                        potential_modules.insert(0, "lfx.components.input_output.text")
                    
                    if potential_modules:
                        
                        for mod_path in potential_modules:
                            try:
                                import importlib
                                ...
                                if hasattr(mod, component_class_name):
                                    ...
                                    if mod_file and os.path.exists(mod_file):
                                        ...
                                        logger.info(f"[SERIALIZE] Got source code from module file: {mod_file} (module: {mod_path})")
                                        break
                            except (ImportError, AttributeError, FileNotFoundError):
                                continue
                    
                    # Fallback: try to get file path directly (but skip validation files)
                    if not component_code:
                        class_file = inspect.getfile(component_class)
                        if class_file and "lfx" in class_file and "validate.py" not in class_file:
                            # Read the file directly
                            with open(class_file, 'r') as f:
                                component_code = f.read()
                            logger.info(f"[SERIALIZE] Got source code from file: {class_file}")
                except Exception as e3:
                    logger.debug(f"[SERIALIZE] Could not get code from file/module: {e3}")
            
            # Last resort: try existing _code ONLY if it's not from validation module
            if not component_code and hasattr(self, "_code") and self._code:
                # Check if _code contains the actual component class (not validation module)
                if component_class_name in self._code and "ValidationError" not in self._code[:500]:
                    component_code = self._code
                    logger.info(f"[SERIALIZE] Using existing _code for {component_class_name}")
                else:
                    logger.warning(
                        f"[SERIALIZE] Skipping _code - appears to be from wrong module "
                        f"(contains ValidationError, missing {component_class_name})"
                    )
        
        if not component_code:
            logger.error(
                f"[SERIALIZE] FAILED to get component code for {component_class_name}. "
                f"Module={component_module}, File={inspect.getfile(component_class) if hasattr(component_class, '__file__') or hasattr(inspect.getmodule(component_class), '__file__') else 'unknown'}"
            )

        logger.info(
            f"[SERIALIZE] Final serialization: module={component_module}, "
    ...
    def _should_use_executor(self) -> bool:
        """Determine if component should execute via executor node.

        Returns:
            True if executor should be used, False for local execution
        """
        # Always use executor - local execution disabled
        # Check component-level override (for future use if needed)
        if hasattr(self, "_execution_mode"):
            return self._execution_mode != "local"  # Only skip if explicitly set to local

        # Always return True to force executor usage
        return True

    async def _execute_via_executor_api(
        self, method_name: str, is_async: bool, timeout: int = 30
    ) -> Any:
        """Execute component method via executor node API.
        ...
        Raises:
            Exception: If execution fails or executor is unavailable
        """
        executor_url = os.getenv("LANGFLOW_EXECUTOR_NODE_URL", "http://localhost:8000")
        if not executor_url:
            raise ValueError(
                "LANGFLOW_EXECUTOR_NODE_URL not set. "
                "All components must execute via executor node. "
                "Please set LANGFLOW_EXECUTOR_NODE_URL environment variable."
            )

        # Serialize component state
        component_state = self._serialize_for_executor()
        
        # Log what we're sending
        logger.info(
            f"[EXECUTOR] Serialized component state: "
            f"class={component_state['component_class']}, "
            f"module={component_state['component_module']}, "
            f"code_length={len(component_state.get('component_code', '') or '') if component_state.get('component_code') else 0}"
        )
        ...
        async with httpx.AsyncClient(timeout=timeout + 10) as client:
            try:
                logger.info(
                    f"[EXECUTOR] Calling executor node at {executor_url}/api/v1/execute "
                    f"for method '{method_name}' in component '{self.display_name}'"
                )
                response = await client.post(
                    f"{executor_url.rstrip('/')}/api/v1/execute", json=payload
                )
                ...
                if not result_data.get("success", False):
                    ...
                    raise RuntimeError(f"Executor execution failed: {error_msg}")
                ...
                logger.info(
                    f"[EXECUTOR] Method '{method_name}' completed successfully "
                    f"in {execution_time:.3f}s, result type: {result_type}"
                )

                # Try to reconstruct result object if it's a known type
                # For now, return the raw result - deserialization can be enhanced later
                return result

            except httpx.HTTPError as e:
                logger.error(f"[EXECUTOR] HTTP error calling executor: {e}")
                raise RuntimeError(f"Failed to call executor node: {e}") from e
            except Exception as e:
                logger.error(f"[EXECUTOR] Error executing via executor: {e}")
                raise
    ...
    def _set_output_required_inputs(self) -> None:
        for output in self.outputs:
            ...
            try:
                ...
                logger.debug(
                    "[COMPONENT EXECUTION] Source code for method '%s' in component '%s':\n%s",
                    output.method,
                    self.display_name,
                    source_code,
                )
                ...
                logger.debug(
                    "[COMPONENT EXECUTION] Using fallback code for method '%s' in component '%s'",
                    output.method,
                    self.display_name,
                )
    ...
    async def _get_output_result(self, output):
        ...
        try:
            ...
            logger.debug(
                "[COMPONENT EXECUTION] Could not get source code for method '%s' in component '%s': %s",
                output.method,
                self.display_name,
                e,
            )
        ...
        logger.info(
            "[COMPONENT EXECUTION] Executing method '%s' for output '%s' in component '%s' (async=%s)",
            output.method,
            output.name,
            self.display_name,
            is_async,
        ...
        try:
            # Always use executor node - local execution disabled
            logger.info(
                f"[EXECUTOR] Routing execution to executor node for method '{output.method}'"
            )
            result = await self._execute_via_executor_api(
                method_name=output.method, is_async=is_async, timeout=30
            )
            # Local execution commented out - all components execute via executor
            # result = await method() if is_async else await asyncio.to_thread(method)
            logger.info(
                "[COMPONENT EXECUTION] Method '%s' completed. Result type: %s, Result preview: %s",
                output.method,
            ...
            logger.error(
                "[COMPONENT EXECUTION] TypeError executing method '%s' in component '%s': %s",
                output.method,
                self.display_name,
                e,
            )
            ...
            logger.error(
                "[COMPONENT EXECUTION] Exception executing method '%s' in component '%s': %s: %s",
                output.method,
                self.display_name,
                type(e).__name__,
                e,
