    ...
    if module_name in ("lfx.custom.validate", "lfx.custom.custom_component.component"):
        ...
        if class_name in _component_map:
            ...
            try:
                ...
                logger.info(
                    f"Successfully loaded {class_name} from mapped module {correct_module}"
                )
                ...
                if component_code:
                    ...
                    try:
                        return await load_component_from_code(
                            component_code, class_name
                        )
                    except Exception as code_error:
                        logger.error(
                            f"Code execution also failed for {class_name}: {code_error}"
                        )
            ...
            logger.warning(
                f"Component {class_name} not found in components.json mapping"
            )
...
async def load_component_from_code(component_code: str, class_name: str) -> type:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Error executing component code: {type(e).__name__}: {e}",
        )
...
def reconstruct_tool(value: Any) -> BaseTool | None:
    ...
    try:
        ...
        if "_component_state" in reconstructed.metadata:
            ...
            params = (
                component_state.get("parameters", {})
...
async def execute_component(request: ExecutionRequest) -> ExecutionResponse:
    ...
    try:
        ...
        if request.component_state.input_values:
            ...
            for key, value in request.component_state.input_values.items():
                ...
                if deserialized is None:
                    logger.debug(
                        "Skipping None input value for %s to preserve component default",
                        key,
                    )
                    continue
        ...
        if request.component_state.component_class == "AgentComponent":
            ...
            if (
                request.component_state.input_values
                and request.component_state.input_values.get("tools")
            ):
        ...
        if request.component_state.stream_topic:
            ...
            logger.info(
                f"[NATS] Attempting to publish to topic: {topic} with message_id: {message_id}"
            )
            print(
                f"[NATS] Attempting to publish to topic: {topic} with message_id: {message_id}"
            )
            try:
                ...
                if nats_client:
                    logger.info(
                        "[NATS] NATS client obtained, preparing publish data..."
                    )
                    ...
                    logger.info(
                        f"[NATS] ✅ Successfully published result to NATS topic: {topic} with message_id: {message_id}"
                    )
                    print(
                        f"[NATS] ✅ Successfully published result to NATS topic: {topic} with message_id: {message_id}"
                    )
